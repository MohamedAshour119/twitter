...
function Tweet(props: TweetInfo) {
    ...
    return (
        <>
            <div className={`py-3 sm:px-6 px-2 gap-x-2 grid ${isRetweeted ? 'grid-cols-1 gap-y-2' : ''} border-b border-zinc-700/70 `}>
                {(isRetweeted && location?.pathname === `/users/${username}`) &&
                    <Link to={`/users/${username}`} className={`flex items-center gap-x-2 text-zinc-400/70`}>
                        <BsRepeat/>
                        <span className={`text-sm`}>{(username === user?.username && isRetweeted) ? 'You retweeted' : `${username} retweeted`}</span>
                    </Link>
                }
